...
The @values@ parameter is specified using the "property data types":https://github.com/tinkerpop/rexster/wiki/Property-Data-Types format. Please note that if elements identified in @values@ cannot be found the extension will simply ignore the value for purpose of sending the return results.  For example, if a request is made for vertices with three identifiers but Rexster can only locate one of the three then only one vertex will be returned in the results array.
