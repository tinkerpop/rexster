...
| vertices | @http://base/graphs/graph/tp/batch/vertices?values=(list,(1,2,3))@ | retrieve vertices with _id 1, 2 and 3 |
| vertices | @http://base/graphs/graph/tp/batch/vertices?values=(list,(marko,peter))&type=keyindex&key=name@ | retrieve vertices by key index with name of marko and peter |
| edges | @http://base/graphs/graph/tp/batch/edges?values=(list,(5,6,7))@ | retrieve edges with _id 5, 6, and 7 |
| edges | @http://base/graphs/graph/tp/batch/edges?values=(list,((f,0.2),(f,0.5)))&type=index&key=weight@ | retrieve edges by index with weight of 0.2 and 0.5 |

The @values@ parameter is specified using the "property data types":https://github.com/tinkerpop/rexster/wiki/Property-Data-Types format. Please note that if elements identified in @values@ cannot be found the extension will simply ignore the value for purpose of sending the return results.  In other words, if a request is made for vertices with three identifiers but Rexster can only locate one of the three then only one vertex will be returned in the results array.

The @type@ parameter may be one of @id@, @index@, or @keyindex@.
