...
# The location of the actual Rexster Home (the one under the _target_ folder), for example _D:\rexster-0.5\target\rexster-?.?-standalone_
...
```bat
>installService.bat "C:\Program Files\Java\jdk1.6.0_26\jre\bin\server\jvm.dll" D:\rexster-?.?\target\rexster-?.?-standalone
```
