...
| void       | @/graphs/<graph>/indices/index?key=<key>&value=<value>&class=vertex&id=<id>@    | put vertex with id @<id>@ into @index@ at @<key>/<value>@                                                                       |
...
In all cases, parameters may be POSTed or PUTed on the [[URI|Mapping a URI to JSON]], as form data or as JSON using one of the [[Rexster MIME Types]].
