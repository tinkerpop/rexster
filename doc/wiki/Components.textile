...
Once these components are properly set up within Rexster, then Rexster is ready to serve results to application requests. Moreover, Rexster provides low-level reading/writing operations of the underlying Blueprints graph database through REST (see [[Basic REST API]]).  Rexster also supplies a browser-based interface called [[The Dog House]], which provides access to a mechanism for browsing the elements of configured graphs and a Gremlin Console from which Gremlin scripts can be executed.