...
h2. Getting Results with Data Types

Getting results with data types attached is accomplished through the @showTypes@ parameter.  This value is set to @false@ by default, but when set to @true@ will wrap property results in a JSON object construct showing data type and the value.

```text
http://localhost:8182/gratefulgraph/vertices/337?showTypes=true
```
...
```js
{
    ...
    "results":{
        ...
        "keymap":{"type":"map","value":{"propertyb":{"type":"integer","value":321},"propertya":{"type":"integer","value":123}}},
        "name":{"type":"string","value":"CHINESE BONES"},
        "song_type":{"type":"string","value":"cover"},
        "performances":{"type":"integer","value":1},
        "keylist":{"type":"list","value":[{"type":"integer","value":100},{"type":"integer","value":200},{"type":"string","value":"(i,300"}]},
        "type":{"type":"string","value":"song"},
        "keytyped":{"type":"integer","value":123}},
        "queryTime":1.092324
}
```

h2. Set Properties With Data Types
...
```text
http://localhost:8182/gratefulgraph/vertices/337?keyinteger=123&keystring=xyz
```
...
```text
http://localhost:8182/gratefulgraph/vertices/337?keyinteger=(integer,123)&keystring=xyz
```
...
```text
http://localhost:8182/gratefulgraph/vertices/337?x=(integer,123)&y=(i,123)
```

A long may be specified using @long@ or @l@ as the data type.

```text
http://localhost:8182/gratefulgraph/vertices/337?x=(long,123)&y=(l,123)
```

A float may be specified using @float@ or @f@ as the data type.

```text
http://localhost:8182/gratefulgraph/vertices/337?x=(float,123)&y=(f,123)
```

A double may be specified using @double@ or @d@ as the data type.

```text
http://localhost:8182/gratefulgraph/vertices/337?x=(double,123)&y=(d,123)
```
...
```text
http://localhost:8182/gratefulgraph/vertices/337?keylist=(list,((i,100),(i,200),(i,300)))
```

```text
http://localhost:8182/gratefulgraph/vertices/337?keylist=(list,((i,100),200,(d,300.5)))
```
...
```text
http://localhost:8182/gratefulgraph/vertices/337?keymap=(map,(propertya=(i,123),propertyb=(d,321.5)))
```

h3. Embedded Lists and Maps

Maps and lists can embed other maps and lists within them to virtually any depth required.

```text
http://localhost:8182/gratefulgraph/vertices/337?keylist=(list,((i,100),(i,200),(i,300),(list,(x,y,z)))
```

```text
http://localhost:8182/gratefulgraph/vertices/337?keymap=(map,(propertya=(i,123),propertyb=(d,321.5),propertyc=(list,(x,y,z)),propertyd=(map,(x=xyz)))
```