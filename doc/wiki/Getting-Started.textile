...
h2. Starting the Rexster Web Server
...
```text
rexster-start.sh -configuration other.xml
```
...
[[The Dog House]] provides for browser-based access to some of the underlying Rexster services (basic vertex/edge browsing and [[Gremlin|Using Gremlin]] console) and by default is available at: 

@http://localhost:8183@

!https://github.com/tinkerpop/rexster/raw/master/doc/images/rexster-dog-house-browse.png!

h2. Stopping the Rexster Web Server

To shutdown the web server, execute the @rexster-stop.sh@ script. This will properly release resources and shutdown the server and exit.  The following will appear in the server console: 
