...
The @<web-server-configuration>@ element contains a collection of properties passed to the Jersey runtime and on initialization.  The key element in the set for @<web-server-configuration>@ is the @<com.sun.jersey.config.property.packages>@ element which should contain a semi-colon separated list of packages which inform Jersey where to search for resource classes.
