...
The @<rexster-server-port>@ allows configuration of the port on which Rexster will serve.  The @<rexster-shutdown-port>@ and @<rexster-shutdown-host>@ allows configuration of the port and host on which Rexster will accept shutdown requests.  The @<rexpro-server-port>@ represents the port on which the [[Rexster Console]] communicates to the Rexster server.  This port utilizes RexPro, short for Rexster Protocol, which is a binary protocol.  
...
The @<graph-location>@ element contains the location of the TinkerGraph.  If this value is not specified, then the TinkerGraph will behave as an in-memory graph only.
