[[http://www.psychologytoday.com/files/u162/dog_and_rainbow.png|width=250px]]
...
The extension is named using @@ExtensionNaming@ annotation where a namespace and name is provided. A method is provided called @evaluatePing@. This method is attached to the @ExtensionPoint.GRAPH@. Thus, when @http://localhost:8182/mygraph/ex/ping@ is resolved, a JSON object is returned that has the key/value ping/pong.
...
Note what is returned when @http://localhost:8182/mygraph/ex/ping@ is resolved:
...
My annotating the method and its parameters, an API is created. Next, resolve @http://localhost:8182/mygraph/ex/ping?reply=ping-a-ling@ and what is returned is:
